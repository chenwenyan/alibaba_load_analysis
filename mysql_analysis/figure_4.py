...
def graph():
    ...
    try:
        cursor.execute("select task_id, plan_cpu, plan_mem from batch_task where plan_cpu > 0")
        records = cursor.fetchall()
        plan_result = list(records)
        print(plan_result)
        cursor.execute("select task_id, avg(real_cpu_avg), avg(real_mem_avg) from batch_instance where real_cpu_avg > 0 group by task_id")
    ...
    ax1.set_xlabel('number of CPU request(average per instance)')
    # ax1.set_xlim(0,9)
    ax2.set_ylabel('number of tasks')
    ax2.set_xlabel('numbers of CPU used (average per instance)')

    ax1.bar(unique_avg_cpu, num_of_job_cpu, width=5, color='green')
    ax2.plot(unique_avg_used_cpu, num_of_job_used_cpu, color='green')

    plt.savefig('../imgs_mysql/figure_4.png')
    plt.show()
