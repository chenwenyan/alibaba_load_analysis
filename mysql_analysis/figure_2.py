...
def graph():
    ...
    list_records = []
    try:
        cursor.execute("select count(*) as server_num, time_stamp, max(cpu) as max_cpu,avg(cpu) as avg_cpu, min(cpu) as min_cpu,max(memory) as max_mem, avg(memory) as avg_mem, min(memory) as min_mem from server_usage group by time_stamp")
    ...
    server_num = [x[0] for x in res]
    timestamp = [x[1] for x in res]
    max_cpu = [x[2] for x in res]
    avg_cpu = [x[3] for x in res]
    min_cpu = [x[4] for x in res]
    max_mem = [x[5] for x in res]
    avg_mem = [x[6] for x in res]
    min_mem = [x[7] for x in res]
    ...
    plt.savefig('../imgs_mysql/figure_2.png')
    plt.show()
