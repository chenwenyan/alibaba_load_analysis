...
def graph():
    ...
    res = []
    try:
        cursor.execute("select ts/3600-11 as time_stamp, avg(avg_cpi), avg(avg_mpki) from container_usage group by ts having time_stamp <= 4")
        ...
        cursor.execute("select ts/3600-11 as time_stamp, avg(avg_cpi), avg(avg_mpki) from container_usage group by ts having time_stamp > 4 and time_stamp <= 8")
        records = cursor.fetchall()
        list_records = list(records)
        res[:] = map(list, list_records)
        time = [x[0] for x in res]
        ...
        cursor.execute("select ts/3600-11 as time_stamp, avg(avg_cpi), avg(avg_mpki) from container_usage group by ts having time_stamp > 8")
        records = cursor.fetchall()
        list_records = list(records)
        res[:] = map(list, list_records)
        time = [x[0] for x in res]
